...
class Response implements ResponseInterface
{
    ...
    public function __construct($body = 'php://memory', $status = 200, array $headers = [])
    {
        if (!is_string($body) && !is_resource($body) && !$body instanceof StreamInterface) {
            $message = 'Body must be a string stream resource identifier, '
    ...
    private function validateStatus($code)
    {
        if (!is_numeric($code) || is_float($code) || $code < 100 || $code >= 600) {
            $message = sprintf('Invalid status code "%s"; must be an integer between 100 and 599, inclusive', is_scalar($code) ? $code : gettype($code));
            throw new Exception\InvalidArgumentException($message);
        }
    }

    /**
     * Assert header names and values are valid
     *
     * @param array $headers
     * @return void
     */
